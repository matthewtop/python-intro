import pytest
from src.merito_library.encode_decode import (
    encode_base64,
    decode_base64,
    url_encode,
    url_decode,
...
def test_binary_encode():
    ...
    with pytest.raises(TypeError):
        binary_encode(None)
    assert binary_encode("merito library") == (
        "01101101011001010111001001101001011101000110111100100000"
        "01101100011010010110001001110010011000010111001001111001"
    )
    assert binary_encode("Hello World!") == (
