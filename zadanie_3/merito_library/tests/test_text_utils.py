import pytest
from src.merito_library.text_utils import (
    count_words,
    reverse_words,
    reverse_string,
    count_characters,
...
def test_is_palindrome():
    ...
    assert not is_palindrome("merito")
...
def test_find_most_common_word():
    assert find_most_common_word(
        "Hello world hello") == {"word": "hello", "count": 2}
    assert find_most_common_word(
    ...
    assert find_most_common_word("merito library merito") == {
        "word": "merito", "count": 2}
    assert find_most_common_word("testy testy testy") == {
        "word": "testy", "count": 3}
